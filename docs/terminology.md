...
A call from one onchain program to another. For more information, see
...
Onchain programs interpret the [instructions](#instruction) sent inside
of each [transaction](#transaction) on the Solana blockchain. 

Onchain programs are often referred to as "[_smart contracts_](/docs/core/programs.md)" on other
blockchains.
...
Solana's parallel [onchain program](#onchain-program) run-time.
